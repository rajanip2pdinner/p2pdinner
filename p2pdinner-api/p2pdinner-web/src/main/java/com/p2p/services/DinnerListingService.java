...
import com.p2p.domain.transformers.DinnerListingTransformer;
import com.p2p.domain.vo.DinnerListingVO;
import com.p2p.domain.vo.P2PResponseBuilder;
import com.p2p.domain.vo.P2PResponseBuilderImpl;
...
public class DinnerListingService {
	...
	@Produces(MediaType.APPLICATION_JSON)
	@GET
	public Response viewListingsByDate(@PathParam("profileId") Integer profileId, @QueryParam("inputDate") long inputDate) throws P2PDinnerException {
		Response response;
		try {
			Collection<DinnerListing> currentListings = dinnerListingDataService.listingByDate(profileId, new java.util.Date(inputDate));
			if ( currentListings == null || currentListings.isEmpty()) {
				throw excepBuilder.createException(ErrorCode.NO_LISTINGS, null, Locale.US);
			}
			response = Response.ok(currentListings).build();
