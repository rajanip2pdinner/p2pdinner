...
- (void)awakeFromNib {
    [super awakeFromNib];
    [self initFromStoryBoard];
}

-(void)initFromStoryBoard{
    
    UIView *view=[self initWithFrame:self.frame
                           andRating:0
                           withLabel:NO
                            animated:YES];
    [view setNeedsLayout];
           ...
           animated:(BOOL)animated
{
    ...
    if (self) {
        ...
        [tintLayer setBackgroundColor:[UIColor colorWithRed:237.0/255.0
                                                      green:134.0/255.0
                                                       blue:0.0/255.0
                                                      alpha:1].CGColor];
        ...
        if (self.animated) {
                ...
                timer   = [NSTimer scheduledTimerWithTimeInterval:0.01
                                                           target:self
                                                         selector:@selector(increaseRating)
                                                         userInfo:nil
                                                          repeats:YES];
            
                [self performSelector:@selector(ratingDidChange)
                           withObject:nil
                           afterDelay:0.1];
...
-(void)ratingDidChange
{
    if (self.userRating < 20.0f) {
        ...
        [self.tintLayer setBackgroundColor:[UIColor colorWithRed:237.0/255.0
                                                           green:134.0/255.0
                                                            blue:0.0/255.0
                                                           alpha:1].CGColor];
...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (CGRectContainsPoint(self.bounds, [[[touches allObjects]lastObject] locationInView:self])) {
        ...
        if ([_delegate respondsToSelector:@selector(updatedRatingValue:withCartId:)]) {
            [_delegate updatedRatingValue:_userRating withCartId:_cartId];
        }
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (CGRectContainsPoint(self.bounds, [[[touches allObjects]lastObject] locationInView:self])) {
        ...
        if ([_delegate respondsToSelector:@selector(updatedRatingValue:withCartId:)]) {
            [_delegate updatedRatingValue:_userRating
                               withCartId:_cartId];
        }
